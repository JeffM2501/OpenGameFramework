...
namespace string_util {
	...
	*/
	...
	/** trim the leading whitespace from a string
	*/
	inline std::string trim_whitespace ( const std::string &text )
	{
		...
		bool isSpace = true;
		int iOffset = 0;
		while ( itr != temp.end() && isSpace)
		{
			isSpace = isWhitespace(*itr++);
			if ( isSpace )
				iOffset++;
		}
		if (isSpace)// the entire thing was whitespace
			return std::string("");

		return std::string ( temp.c_str()+ iOffset);
	}
