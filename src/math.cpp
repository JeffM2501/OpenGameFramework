...
float C3DVertex::IncrementX ( float x )
{
	m_rVertex.x += x;
	return m_rVertex.x;
}

float C3DVertex::IncrementY ( float y )
{
	m_rVertex.y += y;
	return m_rVertex.y;
}

float C3DVertex::IncrementZ ( float z )
{
	m_rVertex.z += z;
	return m_rVertex.z;
}

float C3DVertex::ScaleX ( float x )
{
	m_rVertex.x *= x;
	return m_rVertex.x;
}

float C3DVertex::ScaleY ( float y )
{
	m_rVertex.y *= y;
	return m_rVertex.y;
}

float C3DVertex::ScaleZ ( float z )
{
	m_rVertex.z *= z;
	return m_rVertex.z;
}

float C3DVertex::LimitX ( float min, float max  )
{
	if ( m_rVertex.x < min)
		m_rVertex.x = min;
	else if ( m_rVertex.x > max )
		m_rVertex.x = max;

	return m_rVertex.x;
}

float C3DVertex::LimitY ( float min, float max  )
{
	if ( m_rVertex.y < min)
		m_rVertex.y = min;
	else if ( m_rVertex.y > max )
		m_rVertex.y = max;

	return m_rVertex.y;
}

float C3DVertex::LimitZ ( float min, float max  )
{
	if ( m_rVertex.z < min)
		m_rVertex.z = min;
	else if ( m_rVertex.z > max )
		m_rVertex.z = max;
...
bool  C3DVertex::parseTriplet ( const std::string &text, std::string delim )
{
	std::vector<std::string> parts = string_util::tokenize(text,delim);
	if ( parts.size() > 0)
		m_rVertex.z = m_rVertex.y = m_rVertex.x = (float)atof(parts[0].c_str());
	if ( parts.size() > 1)
		m_rVertex.z = m_rVertex.y = (float)atof(parts[1].c_str());
	if ( parts.size() > 2)
		m_rVertex.z = (float)atof(parts[2].c_str());
...
void C3DVertex::inverse ( void )
{
	if (m_rVertex.x != 0)
		m_rVertex.x = 1.0f/m_rVertex.x;
	if (m_rVertex.y != 0)
		m_rVertex.y = 1.0f/m_rVertex.y;
	if (m_rVertex.z != 0)
		m_rVertex.z = 1.0f/m_rVertex.z;
...
bool C3DVertex::operator == (const C3DVertex &p0)
{
		return (m_rVertex.x == p0.m_rVertex.x) && (m_rVertex.y == p0.m_rVertex.y) && (m_rVertex.z == p0.m_rVertex.z);
}
...
void CMatrix::Rotate2d(float angle, float x, float y, float z)
{
	trVertex3D temp;
	temp.x = x;
	temp.y = y;
	temp.z = z;
::Rotate2dMatrix(&matrix,angle,&temp);
}
