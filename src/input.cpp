...
public:
	...
	// for cheap ass ways to get the key state
...
bool CSDLKeyboardDevice:: GetKeyState ( int key )
{
	if (!keymap)
		return false;

	return keymap[key] != 0 ? true : false;
}
...
{
	listCount = 0;
	CEventManager::instance().registerEventHandaler(SDL_MOUSEBUTTONDOWN,SDLMouseDeviceEventCallback,this);
}

CSDLMouseDevice::~CSDLMouseDevice()
{
	CEventManager::instance().removeEventHandaler(SDL_MOUSEBUTTONDOWN,SDLMouseDeviceEventCallback,this);
}
...
float CSDLJoyStick::GetStickValue ( trStickAxisDescriptor *descriptor)
{
	...
	switch(descriptor->part)
	{
		...
		case eAxis:
			{
			short skickRaw = SDL_JoystickGetAxis(theStick,descriptor->partID);
			raw = (float)skickRaw/_STICK_CONSTANT;
...
template <>
CInputMananger* Singleton<CInputMananger>::_instance = (CInputMananger*)0;

CInputMananger::CInputMananger()
{
	CEventManager::instance().registerEventHandaler(SDL_KEYDOWN,InputManangerEventCallback,this);
	CEventManager::instance().registerEventHandaler(SDL_KEYUP,InputManangerEventCallback,this);
	CEventManager::instance().registerEventHandaler(SDL_MOUSEBUTTONDOWN,InputManangerEventCallback,this);
	InitStickSystem();
	info = new trInputInfo;

	relitiveMouseMode = false;
}

CInputMananger::~CInputMananger()
{
	CEventManager::instance().removeEventHandaler(SDL_KEYDOWN,InputManangerEventCallback,this);
	CEventManager::instance().removeEventHandaler(SDL_KEYUP,InputManangerEventCallback,this);
	CEventManager::instance().removeEventHandaler(SDL_MOUSEBUTTONDOWN,InputManangerEventCallback,this);
	Release();
	ReleaseStickSystem();
}
...
bool CInputMananger::GetKeyState ( KEYTypes key )
{
	CSDLKeyboardDevice	*device = (CSDLKeyboardDevice*)info->stdDevices[0];

	return device->GetKeyState((int)key);
}

void CInputMananger::SetMouseRelitiveMode ( bool mode )
{
	relitiveMouseMode = mode;
	if (relitiveMouseMode)
	{
		C3dView	&engine = C3dView::instance();

		SDL_WarpMouse(engine.GetScreenCenter(eXAxis),engine.GetScreenCenter(eYAxis));
	}
}

void CInputMananger::GetMousePos ( int &iX, int &iY, bool bInc )
{
	if (relitiveMouseMode)	// we do this ourselves because we can't let the mouse go outside of the window
	{
		C3dView	&engine = C3dView::instance();

		int newPos[2];
		SDL_GetMouseState(&newPos[0],&newPos[1]);
		iX = newPos[0] - engine.GetScreenCenter(eXAxis);
		iY = newPos[1] - engine.GetScreenCenter(eYAxis);
		SDL_WarpMouse(engine.GetScreenCenter(eXAxis),engine.GetScreenCenter(eYAxis));
	}
	else
	{
