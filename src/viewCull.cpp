...
void CViewPlane::Set ( float fA, float fB, float fC , float fD )
{
	m_bSet = true;
	m_rNorm.x = fA;
	m_rNorm.y = fB;
...
void CViewPlane::Normalise ( void )
{
	m_fODist /= get_mag(m_rNorm);
	set_mag(m_rNorm,1.0f);
}

float CViewPlane::PointDist ( float fX, float fY, float fZ )
{
	if (!m_bSet)
		return 0.0f;
...
bool CViewPlane::PointBehind ( float fX, float fY, float fZ )
{
	if (!m_bSet)
		return false;
...
bool CViewFrustum::PointIn( float fX, float fY, float fZ )
{
	if (!m_bSet)
		return false;
...
bool CViewFrustum::SphereIn( float fX, float fY, float fZ, float fRad )
{
	if (!m_bSet)
		return false;
...
bool CViewFrustum::CubeIn( float fX, float fY, float fZ, float fSize )
{
	if (!m_bSet)
		return false;
...
bool CViewFrustum::BoxIn( float fX, float fY, float fZ ,float fXSize, float fYSize, float fZSize )
{
	if (!m_bSet)
		return false;
...
bool CViewFrustum::QuadOut ( float *fp1, float *fp2, float *fp3, float *fp4 )
{
	if (!m_bSet)
		return false;
